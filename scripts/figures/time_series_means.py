# -*- coding: utf-8 -*-
"""
Created on Mon Oct 14 14:21:30 2024

@author: mngma
"""

#script to load different year means for each region and method 
import xarray as xr
import numpy as np

def year_monthly_means(yearly_sit_list,BOC_yearly_sit_list,ERM_yearly_sit_list,ZIF_yearly_sit_list,yearly_sicc_list,yearly_fdm_list,yearly_CS2WFA_list,year_value):
    ...
    for sit_ds in yearly_sit_list[year_value]:
        ds_list = []
        for i in range(0,7):
    ...
    for sit_ds in BOC_yearly_sit_list[year_value]:
        ds_list = []
        for i in range(0,7):
            ds =  (sit_ds.where((sit_ds.lons >region[i][0])&(sit_ds.lons <region[i][1])))
            ds_list.append(ds)
    ...
    for sit_ds in ERM_yearly_sit_list[year_value]:
        ds_list = []
        for i in range(0,7):
            ds =  (sit_ds.where((sit_ds.lons >region[i][0])&(sit_ds.lons <region[i][1])))
            ds_list.append(ds)
    ...
    for sit_ds in ZIF_yearly_sit_list[year_value]:
        ds_list = []
        for i in range(0,7):
            ds =  (sit_ds.where((sit_ds.lons >region[i][0])&(sit_ds.lons <region[i][1])))
            ds_list.append(ds)
    ...
    for sit_ds in yearly_sicc_list[year_value]:
        ds_list = []
        for i in range(0,7):
            ds =  (sit_ds.where((sit_ds.lons >region[i][0])&(sit_ds.lons <region[i][1])))
            ds_list.append(ds)
    ...
    for sit_ds in yearly_fdm_list[year_value]:
        ds_list = []
        for i in range(0,7):
            ds =  (sit_ds.where((sit_ds.lons >region[i][0])&(sit_ds.lons <region[i][1])))
            ds_list.append(ds)
    ...
    for sit_ds in yearly_CS2WFA_list[year_value]:
        ds_list = []
        for i in range(0,7):
            sit_ds.coords['lons'] = ((sit_ds.coords['lons'] + 180) % 360) - 180
            ds =  (sit_ds.where((sit_ds.lons >region[i][0])&(sit_ds.lons <region[i][1])))
            ds_list.append(ds)
        sit_CS2WFA_list.append(ds_list) 

    RS = []
    for i in range(0,12):
        rs_ds = xr.merge([sit_CS2WFA_list[i][4],sit_CS2WFA_list[i][5]])
        RS.append(rs_ds.sea_ice_thickness)
        
    for i in range(0,12):
        sit_CS2WFA_list[i].pop(4)
        sit_CS2WFA_list[i].pop(4)
        sit_CS2WFA_list[i].insert(4,RS[i])
    ...
    for i in range(0,12):
        ...
        for j in range(0,6):
            ...
            stds.append(round(np.nanstd(ds[j]),2))
            pers.append(round(np.nanpercentile(ds[j], 95),2))
        sit_df_list_CS2WFA.append(df_list) ; means_list_CS2WFA.append(means); mins_list_CS2WFA.append(mins)
        medians_list_CS2WFA.append(medians) ; pers_list_CS2WFA.append(pers) ; stds_list_CS2WFA.append(stds)
    ...
    # Rearrange the data
    for i in range(0,12):
        AB.append(means_list_CS2WFA[i][0])
        WS.append(means_list_CS2WFA[i][1])
        KH.append(means_list_CS2WFA[i][2])
        EA.append(means_list_CS2WFA[i][3])
        RS.append(means_list_CS2WFA[i][4])
        SH.append(means_list_CS2WFA[i][5])

    # Final rearranged list
    means_list_CS2WFA = [AB,WS,KH,EA,RS,SH]
    return (means_list_zrif,means_list_xoc,means_list_boc,means_list_OLM,means_list_sicc,means_list_fdm,means_list_CS2WFA)
